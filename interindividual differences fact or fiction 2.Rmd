...
=======
The subjects in blue were observed to respond only, those in red observed not to respond.
>>>>>>> 8a47102b8061d03295a2755fbc561453661024e6
>>>>>>> 00aecb9335f0dae23eb7df4bd29b9d03dda196d1
 
```{r trt data , echo=FALSE   }
...
No of red points `r mean(foo > -clin.rel.diff)*length(foo)` in treatment arm. So `r mean(foo > -clin.rel.diff)` is the proportion that fail to show clinically relevant difference. In the control arm (similar plot to treatment arm plot not shown) `r mean(foox < -clin.rel.diff)*length(foox)` observed responders. So `r mean(foox < -clin.rel.diff)` is the proportion that show clinically relevant difference in the control arm.
...
<<<<<<< HEAD
The subjects in blue were observed to respond only, those in red observed not to respond.
No of red points `r (mean(foo > -clin.rel.diff)*length(foo))` in treatment arm. So `r p2(mean(foo > -clin.rel.diff))` is the proportion that fail to show clinically relevant difference. In the control arm (similar plot to treatment arm plot not shown) `r (mean(foox < -clin.rel.diff)*length(foox))` observed responders. So `r p2(mean(foox < -clin.rel.diff))` is the proportion that show clinically relevant difference in the control arm.
=======
No of red points `r mean(foo > -clin.rel.diff)*length(foo)` in treatment arm. So `r mean(foo > -clin.rel.diff)` is the proportion that fail to show clinically relevant difference. In the control arm (similar plot to treatment arm plot not shown) `r mean(foox < -clin.rel.diff)*length(foox)` observed responders. So `r mean(foox < -clin.rel.diff)` is the proportion that show clinically relevant difference in the control arm.
...
Based on 'Individual response to treatment: is it a valid assumption?' S Senn BMJ Vol 329 2004
